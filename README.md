...
To build from source instead, see the [Getting Started](docs/getting-started.md) guide.
...
`Minimize("value")` tells HOLA to minimize the `"value"` field returned by the objective function.
`study.run(forrester, n_trials=50)` automates 50 ask/tell iterations.
`study.top_k(1)` returns the best `CompletedTrial`, which carries `.score_vector`, `.params`, and `.metrics`.
...
hola serve study.yaml --port 8000

# Terminal 2 --- run a worker
hola worker --server http://localhost:8000 --exec "python train.py"
```

Workers receive parameters via the `HOLA_PARAMS` environment variable and print a JSON metrics object to stdout. See the [CLI guide](docs/cli-guide.md) for details.
